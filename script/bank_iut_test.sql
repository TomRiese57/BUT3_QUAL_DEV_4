...
-- (créée EN PREMIER car `Compte` en dépend)
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `carte_bancaire` (
    ...
    -- FK vers `Compte` (majuscule — casse identique à la table créée ci-dessus)
    CONSTRAINT `fk_carte_compte` FOREIGN KEY (`numero_compte`) REFERENCES `Compte` (`numeroCompte`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
VALUES
    ...
    ('9876543210987654', 'LA1021931215', 1, 2000,  800, 300,   0, 300, 0, DATE_ADD(CURDATE(), INTERVAL 3 YEAR)),
    ('1111222233334444', 'MD8694030938', 0, 1500,  500,   0,   0,   0, 1, DATE_ADD(CURDATE(), INTERVAL 2 YEAR));
